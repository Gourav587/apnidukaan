...
const SYSTEM_PROMPT = `You are ApniDukaan Bot 🤖 — a friendly assistant for a kirana (grocery) store in Dinanagar, Punjab, India.
...
- If you don't know something, say "Main is baare mein sure nahi hoon, please store ko WhatsApp karein 📱"`;
  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 429) {
        return new Response(JSON.stringify({ error: "Bot thoda busy hai, please thodi der baad try karein 🙏" }), {
          status: 429,
